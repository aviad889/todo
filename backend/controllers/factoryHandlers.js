...
exports.deleteOne = Model =>
    asyncHandler (async(req, res, next) => {
        ...
        if(!deletedDoc) return next(new AppError(400, 'The requested docs is not found'));
        res.status(204).json({
            status:'success',
            doc: null,
        });
...
exports.getOne = Model => 
    asyncHandler(async(req, res, next) => {
    if(!req.params.id) return next(new AppError(403, 'The Id is not provided'));
    const doc = await Model.findById(req.params.id);
    if (!doc) return next(new AppError(400, 'The requested docs is not found'));
    res.status(200).json({
        status: "success",
        doc,
      });
